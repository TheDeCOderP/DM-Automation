  ...
  try {
    ...
    const tokenRes = await fetch('https://open.tiktokapis.com/v2/oauth/token/', {
      ...
      headers: { 
        ...
        // Note: TikTok recommends passing client_key/secret in the body, not Authorization header
      },
      body: new URLSearchParams({
        grant_type: 'authorization_code',
        code,
        redirect_uri: redirectUri,
        client_key: process.env.TIKTOK_CLIENT_ID!, // TikTok Client Key
        ...
        const account = await prisma.socialAccount.upsert({
          where: {
            platform_platformUserId: {
              platform: 'TIKTOK',
              platformUserId: open_id // Use TikTok's unique open_id for this app
            }
          },
          update: {
